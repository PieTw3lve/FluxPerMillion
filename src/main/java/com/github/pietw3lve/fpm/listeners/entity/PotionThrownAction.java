...
public class PotionThrownAction implements EventActionUtil<ProjectileHitEvent> {
    ...
    public void execute(ProjectileHitEvent event) {
        ...
        FluxLevelChangeEvent fluxEvent = new FluxLevelChangeEvent(plugin.getFluxMeter(), projectile.getLocation(), player, "thrown", projectile.getName().toLowerCase(), points, ActionCategory.POLLUTION);
